...
if autenticado:
    ...
    def carregar_planilha(uploaded_file):
        for header_row in [0, 1, 2, 3]:
            df_try = pd.read_excel(uploaded_file, header=header_row)
            if 'SKU' in df_try.columns and 'Nome' in df_try.columns:
                return df_try
        return None
    ...
    if file:
        ...
        if df is None:
            st.error("❌ Não foi possível identificar os cabeçalhos da planilha.")
        else:
            df.rename(columns={
                "Nome": "Produto",
                ...
                "Desp. Incompleto": "Desp. Incompleto",
                "Vendas": "Vendas",
                "Total": "Total",
