...
autenticador = stauth.Authenticate(
    credentials, "meu_app", "chave_super_secreta", cookie_expiry_days=1
)
...
if autenticado:
    ...
    if file:
        ...
        skus_todos = [...]  # sua lista completa aqui (mantida caso a planilha traga todos)

        # Verifica se só existem SKUs críticos na planilha
        contem_apenas_criticos = df['SKU'].isin(skus_criticos).all()

        exibir_criticos = st.checkbox("Exibir Itens Críticos", value=True)
        if not contem_apenas_criticos:
        ...
        if not df_filtrado.empty:
            ...
            for i in range(len(df_final)):
                for j, col_name in enumerate(df_final.columns):
                    ...
                    table[(i + 1, j)].set_facecolor(get_color(valor, col_name))
